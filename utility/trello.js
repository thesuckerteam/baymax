...
exports.addNewCard = function (listid, name, desc, callback) {
    trello.addCard(name, desc, listid,
        function (error, trelloCard) {
            if (error) {
                console.log(error)
            } else {
...
exports.addTeamMemberToBoard = function(memberId, callback) {
    trello.addMemberToBoard('5df342eb9a2497638c2e7d21', memberId, function(error, result){
        if (error) console.log('error : ' + error)
        else console.log(result)
...
exports.addTeamMemberToCard = function(cardId,memberId,callback) {
    trello.addMemberToCard(cardId, memberId, function(error, result){
        if (error) console.log('error : ' + error)
        else console.log(result)
        callback(error, result)
    })
...
exports.addDueDateToCard = function(cardId, dateValue,callback) {
    trello.addDueDateToCard(cardId, dateValue, function(error, result){
        if (error) console.log('error : ' + error)
        else console.log(result)
        callback(error, result)
    })
